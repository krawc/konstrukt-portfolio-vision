...
interface PortfolioEmailRequest {
  userResponse: string;
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { userResponse }: PortfolioEmailRequest = await req.json();

    if (!userResponse || userResponse.trim() === "") {
      return new Response(
        JSON.stringify({ error: "User response is required" }),
        {
          status: 400,
